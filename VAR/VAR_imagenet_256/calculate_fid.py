#!/usr/bin/env python3
# Single-model Neon VAR FID/IS eval (quiet, DDP-safe, no merge, prints one line)
# Auto-downloads VAE if not provided/found.

import os
# Quiet worker logs (torchrun banner still needs OMP set before launch if you want it gone)
...
# Ensure PYTHONPATH points to VAR/VAR_imagenet_256
...
def build_balanced_labels(num_images: int, num_classes: int, seed: int) -> torch.Tensor:
    per = num_images // num_classes
    labels = torch.arange(num_classes).repeat_interleave(per)
...
def download_file(url: str, dest: Path):
    ...
    try:
        ...
        pass
    # Fallback to wget/curl if available
...
def parse_args():
    ...
    p.add_argument("--model_depth", type=int, default=16, choices=[16], help="VAR depth (d16)")
    ...
    # If omitted or path doesn't exist, we'll auto-resolve/download the official VAE.
...
def main():
        ...
        if master:
            ...
            try:
                ...
                raise FileNotFoundError(
                    "VAE checkpoint not found locally and auto-download failed. "
                    f"Attempted URL: {url}\nError: {e}"
                )
    ...
    with autocast(device_type="cuda", dtype=torch.float16, enabled=True):
        for _ in pbar:
            ...
            if cur_B <= 0:
                continue  # keep loop length consistent across ranks
