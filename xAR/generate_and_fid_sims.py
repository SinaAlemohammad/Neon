...
#
# Single-model FID/IS eval (quiet, DDP-safe):
# - No merging. ONE model + KL-VAE
# - Device-aware DDP barriers (no NCCL "devices unknown")
# - No early rank exit during sampling (prevents NCCL timeouts)
# - torch.amp.autocast('cuda'), safe torch.load (weights_only if available)
# - Always CUDA metrics for torch_fidelity
# - Prints ONE clean metrics line; deletes temp images
# - Graceful DDP teardown (no "process group not destroyed" warning)

import os
# Quiet env for worker processes (launcher banner still needs OMP at shell)
os.environ.setdefault("TORCH_CPP_LOG_LEVEL", "ERROR")
os.environ.setdefault("NCCL_DEBUG", "ERROR")
os.environ.setdefault("PYTHONWARNINGS", "ignore")
os.environ.setdefault("OMP_NUM_THREADS", "1")

import argparse, shutil, random, contextlib, warnings
...
def parse_args():
    ...
    p.add_argument('--no_isc', action='store_true')
    p.add_argument('--cuda', action='store_true')  # ignored; we force CUDA below
    ...
    p.add_argument('--out_dir', type=str, default='temp_images')
...
def main():
    ...
    # Establish local rank & select device BEFORE DDP init
    ...
    # Device-aware barrier helper (prevents NCCL "devices unknown")
    def ddp_barrier():
    ...
    model = net = vae = None  # for teardown
    ...
    try:
        ...
        for b in it:
            ...
            # IMPORTANT: do NOT break early; keep all ranks in lockstep to hit the next barrier
            if idx0 >= labels.numel():
                # should not happen due to padding, but keep safe
                continue
        ...
        if rank == 0:
            ...
            m = torch_fidelity.calculate_metrics(
                ...
                fid_statistics_file=stats, cuda=True,
